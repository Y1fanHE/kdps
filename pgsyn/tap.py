'''
Author: He,Yifan
Date: 2022-02-16 21:42:30
LastEditors: He,Yifan
LastEditTime: 2022-02-18 19:32:52
'''
...
class StdOutRun(Tap):
    ...
    def pre(self, id: str, args, kwargs, obj=None):
        ...
        if self.pre_print_config:
            ...
            for attr in attrs:
                print(attr + ":", getattr(search, attr))
        if self.pre_print_atoms:
            print("Instructions:")
            print(", ".join([i for i in search.spawner.instruction_set.keys()]))
            print("Literals:")
            print(search.spawner.literals)
            print("ERC Generators:")
            print(search.spawner.erc_generators)
        print("========================================")
        print("Start Run")
        print("========================================")
...
def set_verbosity(level: int):
    ...
    """
    if level > 0:
        TapManager.register("pgsyn.gp.genome.GenomeSimplifier.simplify", StdOutSimplification())

    if level == 1:
        TapManager.register("pgsyn.gp.algorithms.base.SearchAlgorithm.step", StdOutSearchStepTap(every_n_steps=5))
        TapManager.register("pgsyn.gp.algorithms.base.SearchAlgorithm.run", StdOutRun(pre_print_config=True,
                                                                              pre_print_atoms=False,
                                                                              post_print_best=False))
    if level > 1:
        TapManager.register("pgsyn.gp.algorithms.base.SearchAlgorithm.step", StdOutSearchStepTap(every_n_steps=1))
        TapManager.register("pgsyn.gp.algorithms.base.SearchAlgorithm.run", StdOutRun(pre_print_config=True,
                                                                              pre_print_atoms=True,
                                                                              post_print_best=True))
        TapManager.register("pgsyn.gp.genome.GenomeSimplifier._step", StdOutSimplificationStep())
