...
LastEditTime: 2022-02-18 20:49:59
'''
...
from pgsyn.push.types import PushType
from pgsyn.push.atoms import Literal
from pgsyn.gp.genome import Genome, GeneSpawner
from pgsyn.knowledge.base import KnowledgeArchive
from pgsyn.tap import tap
from pgsyn.utils import DiscreteProbDistrib, instantiate_using
...
class VariationOperator(ABC):
    ...
    def produce(self, parents: Sequence[Genome], spawner: GeneSpawner, **kwargs) -> Genome:
        new_genome = self._produce(parents, spawner, **kwargs)
        max_genome_size = kwargs.get("max_genome_size")
        if max_genome_size:
            new_genome = new_genome[:max_genome_size]
        return new_genome
...
class InsertionMutation(VariationOperator):
    """Randomly inserts a genome from an archive to parent.
    Parameters
    ----------
    rate : float
        The probability of inserting a given genome at a given point in the
        parent Genome. Default is 0.1.
    Attributes
    ----------
    rate : float
        The probability of inserting a given genome at a given point in the
    ...
    @tap
    def _produce(self, parents: Sequence[Genome], spawner: GeneSpawner, knowledge_archive: KnowledgeArchive, **kwargs) -> Genome:
        """Produce a child Genome from parent Genomes and optional GenomeSpawner.
        Parameters
        ----------
        parents
        ...
        spawner
            A GeneSpawner that can be used to produce new genes (aka Atoms).
        """
        super()._produce(parents, spawner, knowledge_archive, **kwargs)
        self.checknum_parents(parents)
        insertion_location = randint(0, len(parents[0]))
        new_genome = Genome()
        for i, gene in enumerate(parents[0]):
            if i == insertion_location and random() < self.rate:
                for insertion_gene in knowledge_archive.spawn_genome():
                    new_genome = new_genome.append(insertion_gene)
            new_genome = new_genome.append(gene)
        return new_genome


class ReplacementMutation(VariationOperator):
    """Randomly replaces a part of parent with a genome from an archive.
    Parameters
    ----------
    rate : float
        The probability of replacing a part of parent at any given interval
        with a given genome. Default is 0.1.
    Attributes
    ----------
    rate : float
        The probability of replacing a part of parent at any given interval
    ...
    def __init__(self, replacement_rate: float = 0.01):
        super().__init__(1)
    ...
    def _produce(self, parents: Sequence[Genome], spawner: GeneSpawner, knowledge_archive: KnowledgeArchive, **kwargs) -> Genome:
        ...
        spawner
            A GeneSpawner that can be used to produce new genes (aka Atoms).
        """
        super()._produce(parents, spawner, knowledge_archive=knowledge_archive, **kwargs)
        self.checknum_parents(parents)
        replacement_genome = knowledge_archive.spawn_genome()
        if len(replacement_genome) >= len(parents[0]):
            return replacement_genome
        replacement_start_location = randint(0, len(parents[0]) - len(replacement_genome))
        replacement_end_location = replacement_start_location + len(replacement_genome)
        new_genome = Genome()
        for i, gene in enumerate(parents[0]):
            if random() < self.rate:
                if i in range(replacement_start_location, replacement_end_location):
                    for replacement_gene in replacement_genome:
                        new_genome = new_genome.append(replacement_gene)
                else:
                    new_genome = new_genome.append(gene)
            else:
                new_genome = new_genome.append(gene)
